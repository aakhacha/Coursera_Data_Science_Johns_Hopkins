# Statistical Inference - Week 4 - Quiz 4
...
Question 2
----------
A sample of 9 men yielded a sample average brain volume of 1,100cc and a standard deviation of 30cc. What is the complete set of values of µ0 that a test of H0:µ=µ<sub>0</sub> would fail to reject the null hypothesis in a two sided 5% Students t-test?
...
pval <- pt((µ_y - µ_x) / (s_p * (1 / n_x + 1 / n_y)^.5), df=n_y + n_x -2)
pval
```
...
```{r}
n <- 100 #subject
µ <- 0.01# m^3 brain volume loss mean
s <- 0.04# m^3 brain volume loss std. dev.
p <- 0.05 # sign level

pow <- power.t.test(n=n, delta=µ, sd=s , sig.level=p, type="one.sample", alt="one.sided")$power
round(pow, 2)
```
...
n <- power.t.test(power=pow, delta=µ, sd=s , sig.level=p, type="one.sample", alt="one.sided")$n
ceiling(n/10)*10
```
